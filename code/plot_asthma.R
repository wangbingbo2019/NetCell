...
set.seed(1)
epsilon = 1e-10
tissue_core = read.csv("/home/zkl/Desktop/NetCell/trait-cells-in-tissues-and-celltypes/tissues/29/top10_core_lcczscore.csv"
                       , na.strings = "", fileEncoding = "GB18030")
celltype_core = read.csv("/home/zkl/Desktop/NetCell/trait-cells-in-tissues-and-celltypes/celltypes/29/top10_core_lcczscore.csv"
                         , na.strings = "", fileEncoding = "GB18030")[,c('tissue','celltype','fisher.s.pvalue')]
# 找到 "column3" 小于 0.05 的行的索引
rows_to_change <- celltype_core$fisher.s.pvalue > 0.005
# 将符合条件的行的 "column2" 改为 "other"
celltype_core$celltype[rows_to_change] <- "other"
celltype_core$fisher.s.pvalue[rows_to_change] <- 0.005
celltype_core = unique(celltype_core)

d11 = data.frame(from="Asthma", to=tissue_core[['tissue']])
d21 = data.frame(from=celltype_core[['tissue']],
                 to=paste(celltype_core[['tissue']],celltype_core[['celltype']], sep = "---"))
...
ggsave("/home/zkl/Desktop/NetCell/mygraph-en-change.png",
       width = 10,             # 宽
       height = 10,            # 高
       units = "in",          # 单位
       dpi = 500              # 分辨率DPI
